...
def init_squeeze(rng, init_batch=None):
    ...
    def forward(params, prev_sample, prev_logp=0.):
        return squeeze2d(prev_sample), prev_logp

    def reverse(params, next_sample, next_logp=0.):
        return unsqueeze2d(next_sample), next_logp

    return params, forward, reverse
...
def init_conv_affine_coupling(rng, in_shape, n_channels, flip, sigmoid=True, init_batch=None):
    ...
    h, w, c = in_shape
    half_c = c // 2
    net_init, net_apply = stax.serial(Conv(n_channels, 3), Relu, Conv(n_channels, 3), Relu, Conv(c, 3))
    ...
    def forward(net_params, prev_sample, prev_logp=0.):
    ...
    def reverse(net_params, next_sample, next_logp=0.):
    ...
    return net_params, forward, reverse
